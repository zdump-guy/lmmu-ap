...
window.State = {
  ...
  leaderboard: [
    { rank: 1, name: "Player One", xp: 5250 },
    { rank: 2, name: "Sarah", xp: 4850 },
    { rank: 3, name: "Mike", xp: 4100 },
    { rank: 4, name: "Anna", xp: 3900 },
    { rank: 5, name: "James", xp: 2200 },
  ],
};
...
window.App = {
  init: function () {
    if (window.innerWidth < 768) {
      document.getElementById("mobile-menu-btn").style.display = "inline-flex";
    }
  ...
  addXP: function (amount) {
    ...
    if (window.State.user.xp >= window.State.user.nextLevelXp) {
      ...
      window.State.user.nextLevelXp = Math.floor(
        window.State.user.nextLevelXp * 1.5
      );
...
window.toggleSidebar = function (forceOpen) {
  ...
  if (forceOpen === false || sidebar.classList.contains("open")) {
...
window.Quests = {
  render: function (container) {
    container.innerHTML = `<div class="kanban-grid">${this.renderColumn(
      "To Do",
      "todo"
    )}${this.renderColumn("Active", "inprogress")}${this.renderColumn(
      "Done",
      "completed"
    )}</div>`;
    this.setupDragDrop();
  ...
  renderColumn: function (title, status) {
    ...
    return `<div class="flex-col gap-4"><div class="col-header animate-entry" style="animation-delay:0.1s"><span class="font-bold text-navy">${title}</span><span class="badge badge-low">${
      tasks.length
    }</span></div><div id="${status}" class="drop-zone animate-entry" style="animation-delay:0.2s">${tasks
      .map((t, i) => this.renderCard(t, i))
      .join("")}</div></div>`;
  },
  renderCard: function (t, index) {
    ...
    let actionButton = "";
    if (t.status === "todo") {
      actionButton = `<button onclick="Quests.moveTask(${t.id}, 'inprogress')" class="quest-action-btn quest-action-start"><i data-lucide="play" style="width:14px;height:14px;"></i> Start</button>`;
    } else if (t.status === "inprogress") {
      actionButton = `<button onclick="Quests.moveTask(${t.id}, 'completed')" class="quest-action-btn quest-action-done"><i data-lucide="check" style="width:14px;height:14px;"></i> Done</button>`;
    }
    return `<div class="task-card animate-entry" style="animation-delay:${
      index * 0.1 + 0.2
    }s" draggable="true" id="task-${
      t.id
    }"><div class="flex justify-between items-center" style="margin-bottom:0.5rem;"><span class="badge ${badge}">${
      t.priority
    }</span><span style="font-size:0.75rem;" class="font-bold text-gold">${
      t.xp
    } XP</span></div><div style="font-weight:700; margin-bottom:0.5rem;" class="text-navy">${
      t.title
    }</div><div class="flex justify-between items-center" style="gap:0.5rem;"><div style="font-size:0.75rem;" class="text-muted">${
      t.date
    }</div>${actionButton}</div></div>`;
  },
  moveTask: function (taskId, newStatus) {
    const taskIdx = window.State.tasks.findIndex((t) => t.id === taskId);
    if (taskIdx > -1) {
      const oldStatus = window.State.tasks[taskIdx].status;
      if (newStatus === "completed" && oldStatus !== "completed") {
        App.addXP(window.State.tasks[taskIdx].xp);
      }
      window.State.tasks[taskIdx].status = newStatus;
  ...
  setupDragDrop: function () {
    ...
    document.querySelectorAll(".drop-zone").forEach((zone) => {
      ...
      zone.addEventListener("drop", (e) => {
        ...
        if (taskIdx > -1) {
          const currentStatus = window.State.tasks[taskIdx].status;
          const newStatus = zone.id;
          // Prevent backward movements
          if (currentStatus === "inprogress" && newStatus === "todo") return;
          if (
            currentStatus === "completed" &&
            (newStatus === "todo" || newStatus === "inprogress")
          )
            return;
...
window.Leaderboard = {
  render: function (container) {
    ...
    rest.forEach((u, i) => {
      html += `<div class="leaderboard-item"><span class="text-muted font-black" style="width:30px;">#${
        i + 4
      }</span><img src="https://api.dicebear.com/7.x/avataaars/svg?seed=${
        u.name
      }" style="width:40px; border-radius:50%;"><div style="flex:1; font-weight:700;">${
        u.name
      }</div><div class="font-bold text-navy">${u.xp} XP</div></div>`;
    });
    html += `</div></div>`;
...
window.Profile = {
  render: function (container) {
    const u = window.State.user;
    const percent = Math.min((u.xp / u.nextLevelXp) * 100, 100);
    container.innerHTML = `<div style="max-width: 800px; margin: 0 auto;"><div class="profile-hero animate-entry"><div class="profile-deco" style="position: absolute; top: -20px; right: -20px; width: 150px; height: 150px; background: rgba(251, 174, 66, 0.1); border-radius: 50%; filter: blur(40px);"></div><img src="https://api.dicebear.com/7.x/avataaars/svg?seed=${
      u.name
    }" style="width:120px; height:120px; border-radius:50%; background:#f1f5f9; position: relative; z-index: 10;"><div style="flex:1; position: relative; z-index: 10;"><h2 style="font-size:1.5rem;" class="font-black text-navy">${
      u.name
    }</h2><p class="text-muted font-bold" style="margin-bottom:1rem;">${
      u.title
    }</p><div style="background:#f1f5f9; height:12px; border-radius:6px; overflow:hidden;"><div style="width:${percent}%; height:100%; background:linear-gradient(90deg, var(--gold), #fcd34d); transition: width 1s ease;"></div></div><div class="flex justify-between text-muted font-bold" style="font-size:0.75rem; margin-top:0.5rem;"><span>Current Progress</span><span>${
      u.xp
    } / ${
      u.nextLevelXp
    } XP</span></div></div></div><div class="profile-stats-grid">${this.stat(
      "globe",
      "Global Rank",
      "#1",
      0.1
    )}${this.stat("check-circle-2", "Quests Done", "42", 0.2)}${this.stat(
      "bug",
      "Bugs Squashed",
      "128",
      0.3
    )}${this.stat("flame", "Day Streak", "12", 0.4)}</div></div>`;
  },
...
window.Achievements = {
  data: [
    ...
    {
      title: "Warrior",
      desc: "Complete 10 high priority tasks",
      icon: "sword",
      unlocked: true,
    },
    ...
    { title: "Locked in", desc: "Get 5000 XP", icon: "gem", unlocked: true },
    {
    ...
    {
      title: "Bro has no life",
      desc: "Get 10,000 XP",
      icon: "gem",
      unlocked: true,
    },
    {
      title: "Glued to the TERMINAL",
      desc: "Get 20,000 XP",
      icon: "gem",
      unlocked: true,
    },
  ...
  render: function (container) {
    ...
    this.data.forEach((b, i) => {
      const style = b.unlocked
        ? "border:2px solid var(--gold); background:white; cursor:pointer;"
        : "border:2px solid var(--border); background:#f1f5f9; opacity:0.6; filter:grayscale(100%); cursor:pointer;";
      html += `<div class="achievement-card animate-entry" style="${style} animation-delay: ${
        i * 0.1
      }s;" onclick="Achievements.showDetails('${b.title}', '${b.desc}', ${
        b.unlocked
      })"><div style="width:50px; height:50px; margin:0 auto 1rem; border-radius:50%; background:var(--canvas); display:flex; align-items:center; justify-content:center; color:var(--gold);"><i data-lucide="${
        b.icon
      }"></i></div><div class="font-bold text-navy">${
        b.title
      }</div><div style="font-size:0.7rem; font-weight:900; text-transform:uppercase; color:${
        b.unlocked ? "#22c55e" : "#64748b"
      }; margin-top:0.25rem;">${
  ...
  showDetails: function (title, desc, unlocked) {
    ...
    modal.innerHTML = `<div class="modal-content" style="text-align:center; padding: 2rem;"><h3 class="text-navy font-black" style="font-size: 1.5rem; margin-bottom: 0.5rem;">${title}</h3><div style="margin-bottom: 1rem; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; color: ${
      unlocked ? "#22c55e" : "#64748b"
    };">${
      unlocked ? "Unlocked" : "Locked"
    }</div><p class="text-muted">${desc}</p><button class="btn btn-secondary" style="margin-top: 1.5rem;" onclick="document.getElementById('achievement-modal').classList.remove('open')">Close</button></div>`;
...
window.Utils = {
  renderHowTo: function (c) {
    c.innerHTML = `
                    <div class="text-center animate-entry" style="max-width:800px; margin:0 auto; padding-bottom: 5rem;">
